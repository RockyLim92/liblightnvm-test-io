...
void _test_1ADDR(int use_meta)
{
	...
	printf("[hkim] buf_w_nbytes %d buf_r_nbytes %d\n", buf_w_nbytes, buf_r_nbytes);
		...
		for (size_t pl = 0; pl < geo->nplanes; ++pl) {
			for (size_t sec = 0; sec < geo->nsectors; ++sec) {
				...
				if (use_meta) {
					...
					meta_diff = compare_meta_buffers(meta_r, meta_w + mw_offset, meta_r_nbytes);
				}
...
void _test_NADDR(int use_meta, int pmode)
{
	...
	printf("[hkim] meta_nbytes : %d \n", meta_nbytes);		// 256 for QUAD
	if (!meta_w) {
		CU_FAIL("nvm_buf_alloc");
		goto exit_naddr;
	}
	...
	for (int i = 0; i < naddrs; ++i) {
		...
		printf(" strlen(meta_descr) %d geo->meta_nbytes %d\n", strlen(meta_descr), geo->meta_nbytes);
		if (strlen(meta_descr) > geo->meta_nbytes) {
			CU_FAIL("Failed constructing meta buffer");
			goto exit_naddr;
		}
	...
	for (size_t pg = 0; pg < geo->npages; ++pg) {
		...
		if (use_meta) {
			//meta_diff = compare_buffers(meta_r, meta_w + mw_offset, meta_r_nbytes);	// hkim meta_r diff
			meta_diff = compare_meta_buffers(meta_w, meta_r, meta_nbytes);
		}
#if 0
		if (use_meta)
			meta_diff = compare_buffers(meta_r, meta_w, meta_nbytes);	// hkim meta_r diff
#endif
		...
		if (use_meta && meta_diff) {
			CU_FAIL("Read failure: meta mismatch");
			printf("[hkim] Read failure: meta mismatch\n");
